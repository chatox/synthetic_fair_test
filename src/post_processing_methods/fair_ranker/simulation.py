...
from src.dataset_creator.candidate import Candidate
from src.post_processing_methods.fair_ranker.test import FairnessInRankingsTester
...
def create(fairnessProbability, k, numProtected, numNonProtected):
    ...
    j = 0
    while protectedCandidates and unprotectedCandidates and (j < k):
...
def generateCandidateList(isProtected, numCandidates):
    ...
    if isProtected:
        ...
        for u in range(numCandidates):
            candidates.append(Candidate(u, ["female"]))
    else:
        for u in range(numCandidates):
            candidates.append(Candidate(u, []))
